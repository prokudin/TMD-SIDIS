...
In the T-even case one obtains the following approximations\footnote{%
	...
	Notice that in our notation transverse vectors
...
For Mellin moments it was found that~\cite{Balla:1997hf,Dressler:1999hc}
...
Notice that this type occurs for all subleading-twist FFs that enter
...
There are three cases: $f_T^a(x,k_\perp)$, $h^a(x,\kperp)$, and $e_L^a(x,\kperp)$.
...
and the transverse momentum of the final hadron produced~\cite{Collins:1992kk}.
...
\includegraphics[width=0.37\textwidth]{\FigPath/ALT_COMPASS_x.pdf} {\tiny (c)}
	...
	Leading-twist $A_{LT}^{\cos(\phi_h-\phi_S)}$:
	 preliminary COMPASS data \cite{Parsamyan:2013fia} (a,b); and our calculation for COMPASS kinematics (c).
	}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hskip -0.2cm\includegraphics[height=3.45cm]{\FigPath/AULsin2phi_COMPASS_x} {\tiny (b)}
...
\includegraphics[height=3.5cm]{\FigPath/AULsin2phi_JLAB_x.pdf} {\tiny (c)}
	...
	JLab \cite{Jawalkar:2017ube} (b),  and from our calculation for COMPASS kinematics in the WW-type
	approximation (c). Results
	from \cite{Avakian:2007mv} are also shown in comparison to the COMPASS data.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
WW-type approximations can be applied to all eight subleading-twist asymmetries,
...
two terms are proportional to the pure twist-3 fragmentation functions
$\tilde{G}^{\perp a}$ and $\tilde{E}^a$ and neglected here; the other two
...
result for this SIDIS function in (\ref{Eq:FLT-collinear})
...
\includegraphics[width=0.35\textwidth]{\FigPath/ALLcosphi_COMPASS_x} {\tiny (b)}
	...
	\cite{Parsamyan:2018ovx,Parsamyan:2018evv} (a),  
	our calculation for COMPASS kinematics using the WW-type approximation (b). }
\end{figure}
%%%%%%%%%%%%%%%%%%%%
...
\caption{\label{autsinphis}
	...
	our calculation for COMPASS kinematics (c), and HERMES data \cite{Schnell:2010zza} (d).}
\end{figure}
...
two terms proportional to $h_T^{\perp q}(x,\kperp)$ and $h_T^q(x,\kperp)$, respectively, 
...
$\int d^2\kperp \,f_T^q(x,\kperp)=0$, see (\ref{Eq:sum-rules-T-odd})
...
$f_T^{\perp q}(x,\kperp)$, $h_T^{\perp q}(x,\kperp)$, and $h_T^q(x,\kperp)$,
...
and $\la\kperp^2\ra_{f_T^\perp}=\la\kperp^2\ra_{f_{1T}^\perp}$. The 
expressions obtained are given by
\begin{subequations}\begin{alignat}{1}
...
\caption{\label{autsin2phihphis}
	Subleading-twist $A_{UT}^{\sin(2 \phi_h - \phi_S)}(x)$: 
	preliminary COMPASS data~\cite{Parsamyan:2013fia} (a,b), our calculation for COMPASS kinematics (c),
	and similarly $A_{UT, \langle y \rangle}^{\sin(2\phi_h-\phi_S)}(x)$ in comparison to
...
Using the Gaussian Ansatz for $f^\perp(x,\kperp)$ in (\ref{eq:fperpnew})
...
as it uses grids for TMDs, PDFs, and FFs. SIDIS cross section examples are presented in Fig.~\ref{fig:package}.
...
A.~Kotzinian, \emph{New quark distributions and semiinclusive electroproduction
  on the polarized nucleons}, {\emph{Nucl. Phys.} {\bf B441} (1995) 234--248},
  [\href{https://arxiv.org/abs/hep-ph/9412283}{{\tt hep-ph/9412283}}].
...
A.~Bacchetta et~al., \emph{{Semi-inclusive deep inelastic scattering at small
  transverse momentum}}, {\emph{JHEP} {\bf 02} (2007) 093},
  [\href{https://arxiv.org/abs/hep-ph/0611265}{{\tt hep-ph/0611265}}].
...
S.~Goertz, W.~Meyer and G.~Reicherz, \emph{{Polarized H, D and He-3 targets for
  particle physics experiments}},
...
S.~Arnold, A.~Metz and M.~Schlegel, \emph{{Dilepton production from polarized
  hadron hadron collisions}},  \href{https://arxiv.org/abs/0809.2262}{{\tt
  0809.2262}}.
...
M.~Burkardt, \emph{{Spin-orbit correlations and single-spin asymmetries}},
  \href{https://arxiv.org/abs/0709.2966}{{\tt 0709.2966}}.

\bibitem{Burkardt:2009rf}
M.~Burkardt, \emph{{The g(2) Structure Function}},
...
R.~Jaffe, \emph{{G(2): The Nucleon's Other Spin Dependent Structure Function}},
  {\emph{Comments Nucl. Part. Phys.} {\bf 19} (1990) 239}.

\bibitem{Balla:1997hf}
J.~Balla, M.~V. Polyakov and C.~Weiss, \emph{{Nucleon matrix elements of higher
  twist operators from the instanton vacuum}},
...
B.~Dressler and M.~V. Polyakov, \emph{{On the twist-three contribution to h(L)
  in the instanton vacuum}},
...
M.~Gockeler, R.~Horsley, W.~Kurzinger, H.~Oelrich, D.~Pleiter et~al., \emph{{A
  Lattice calculation of the nucleon's spin dependent structure function g(2)
  revisited}}, \href{http://dx.doi.org/10.1103/PhysRevD.63.074506}{\emph{Phys.
  Rev.} {\bf D63} (2001) 074506},
  [\href{https://arxiv.org/abs/hep-lat/0011091}{{\tt hep-lat/0011091}}].

\bibitem{Gockeler:2005vw}
{Gockeler, M. and Horsley, R. and Pleiter, D. and Rakow, Paul E.L. and
  Sch\"afer, A. and others}, \emph{{Investigation of the second moment of the
  nucleon's g(1) and g(2) structure functions in two-flavor lattice QCD}},
...
{\scshape E155} collaboration, P.~L. Anthony et~al., \emph{{Precision
  measurement of the proton and deuteron spin structure functions g2 and
  asymmetries A(2)}},
...
A.~Kotzinian, B.~Parsamyan and A.~Prokudin, \emph{{Predictions for double spin
  asymmetry A(LT) in semi inclusive DIS}},
...
R.~Tangerman and P.~Mulders, \emph{{Polarized twist-three distributions g(T)
  and h(L) and the role of intrinsic transverse momentum}},
...
\bibitem{Metz:2004je}
A.~Metz and M.~Schlegel, \emph{{Twist three single spin asymmetries in
...
\bibitem{Mathematica}
W.~R. Inc., ``Mathematica, {V}ersion 11.3.''
...
D.~Gutierrez-Reyes, I.~Scimemi, W.~J. Waalewijn and L.~Zoppi, \emph{{Transverse
  momentum dependent distributions with jets}},
...
I.~Scimemi and A.~Vladimirov, \emph{{Matching of transverse momentum dependent
  distributions at twist-3}},  \href{https://arxiv.org/abs/1804.08148}{{\tt
  1804.08148}}.
...
A.~Efremov and P.~Schweitzer, \emph{{The Chirally odd twist 3 distribution
  e(a)(x)}}, {\emph{JHEP} {\bf 0308} (2003) 006},
  [\href{https://arxiv.org/abs/hep-ph/0212044}{{\tt hep-ph/0212044}}].

\bibitem{Jaffe:1996zw}
R.~L. Jaffe, \emph{{Spin, twist and hadron structure in deep inelastic
  processes}},  \href{https://arxiv.org/abs/hep-ph/9602236}{{\tt
  hep-ph/9602236}}.
...
E.~V. Shuryak, \emph{{The Role of Instantons in Quantum Chromodynamics. 1.
  Physical Vacuum}},
...
{\scshape Spin Muon (SMC)} collaboration, D.~Adams et~al., \emph{{Spin
  asymmetry in muon - proton deep inelastic scattering on a transversely
  polarized target}},
...
M.~Gluck, E.~Reya and A.~Vogt, \emph{{Dynamical parton distributions
...
A.~V. Efremov, K.~Goeke and P.~Schweitzer, \emph{Collins effect in
  semi-inclusive deeply inelastic scattering and in $e^{+} e^{-}$
  annihilation}, {\emph{Phys. Rev.} {\bf D73} (2006) 094025}.
...
Y.~Koike, K.~Tanaka and S.~Yoshida, \emph{{Drell-Yan double-spin asymmetry
  A(LT) in polarized p anti-p collisions: Wandzura-Wilczek contribution}},
...
{H\"agler, Ph. and Musch, B. U. and Negele, J. W. and Sch\"afer, A.},
  \emph{{Intrinsic quark transverse momentum in the nucleon from lattice QCD}},
...
{Musch, Bernhard U. and H\"agler, Philipp and Negele, John W. and Sch\"afer,
  Andreas}, \emph{{Exploring quark transverse momentum distributions with
  lattice QCD}},
...
{Engelhardt, M. and H\"agler, P. and Musch, B. and Negele, J. and Sch\"afer,
  A.}, \emph{{Lattice QCD study of the Boer-Mulders effect in a pion}},
...
{Musch, B. U. and H\"agler, Ph. and Engelhardt, M. and Negele, J. W. and
  Sch\"afer, A.}, \emph{{Sivers and Boer-Mulders observables from lattice
  QCD}}, \href{http://dx.doi.org/10.1103/PhysRevD.85.094510}{\emph{Phys. Rev.}
  {\bf D85} (2012) 094510}, [\href{https://arxiv.org/abs/1111.4249}{{\tt
  1111.4249}}].
...
J.~Green, K.~Jansen and F.~Steffens, \emph{{Nonperturbative renormalization of
  ...
  field}}, \href{http://dx.doi.org/10.1103/PhysRevLett.121.022004}{\emph{Phys.
  Rev. Lett.} {\bf 121} (2018) 022004},
...
{\scshape LHPC} collaboration, LHPC et~al., \emph{{Transverse structure of
...
{\scshape LHPC Collaborations} collaboration, {H\"agler, Ph. and others},
  \emph{{Nucleon Generalized Parton Distributions from Full Lattice QCD}},
...
{\scshape QCDSF} collaboration, M.~Gockeler et~al., \emph{{Quark helicity flip
  generalized parton distributions from two-flavor lattice QCD}},
...
M.~Burkardt and Y.~Koike, \emph{{Violation of sum rules for twist three parton
...
M.~Wakamatsu, \emph{{Polarized structure functions g(2)(x) in the chiral quark
  soliton model}},
...
M.~Schlegel and A.~Metz, \emph{{On the validity of Lorentz invariance relations
  between parton distributions}},
  \href{https://arxiv.org/abs/hep-ph/0406289}{{\tt hep-ph/0406289}}.
...
P.~Schweitzer, \emph{{The Chirally odd twist three distribution function
...
Y.~Ohnishi and M.~Wakamatsu, \emph{{pi N sigma term and chiral odd twist three
...
C.~Cebulla, J.~Ossmann, P.~Schweitzer and D.~Urbano, \emph{{The Twist-3 parton
  distribution function e**a(x) in large-N(c) chiral theory}}, {\emph{Acta
  Phys. Polon.} {\bf B39} (2008) 609--640},
...
\bibitem{Ji:2006ub}
X.~Ji, J.-W. Qiu, W.~Vogelsang and F.~Yuan, \emph{{A unified picture for single
  transverse-spin asymmetries in hard processes}},
...
J.~C. Collins et~al., \emph{Sivers effect in semi-inclusive deeply inelastic
  scattering}, {\emph{Phys. Rev.} {\bf D73} (2006) 014021},
  [\href{https://arxiv.org/abs/hep-ph/0509076}{{\tt hep-ph/0509076}}].
...
{\scshape COMPASS} collaboration, M.~Aghasyan et~al.,
  ...
  \href{https://arxiv.org/abs/1709.07374}{{\tt 1709.07374}}.
...
A.~Bacchetta, F.~Delcarro, C.~Pisano, M.~Radici and A.~Signori,
  ...
  \href{http://dx.doi.org/10.1007/JHEP06(2017)081}{\emph{JHEP} {\bf 06} (2017)
  081}, [\href{https://arxiv.org/abs/1703.10157}{{\tt 1703.10157}}].
...
D.~W. Sivers, \emph{Single spin production asymmetries from the hard scattering
  of point-like constituents}, {\emph{Phys. Rev.} {\bf D41} (1990) 83}.
...
M.~Anselmino et~al., \emph{Extracting the sivers function from polarized sidis
  data and making predictions}, {\emph{Phys. Rev.} {\bf D72} (2005) 094007},
  [\href{https://arxiv.org/abs/hep-ph/0507181}{{\tt hep-ph/0507181}}].

\bibitem{Anselmino:2005an}
M.~Anselmino et~al., \emph{{Comparing extractions of Sivers functions}},
  \href{https://arxiv.org/abs/hep-ph/0511017}{{\tt hep-ph/0511017}}.

\bibitem{Vogelsang:2005cs}
W.~Vogelsang and F.~Yuan, \emph{Single-transverse spin asymmetries: From dis to
  hadronic collisions}, {\emph{Phys. Rev.} {\bf D72} (2005) 054028}.

\bibitem{Anselmino:2008sga}
M.~Anselmino et~al., \emph{{Sivers Effect for Pion and Kaon Production in Semi-
...
M.~Anselmino, M.~Boglione and S.~Melis, \emph{{A Strategy towards the
  extraction of the Sivers function with TMD evolution}},
...
A.~Bacchetta and M.~Radici, \emph{{Two hadron semiinclusive production
  including subleading twist}},
...
\bibitem{She:2009jq}
J.~She, J.~Zhu and B.-Q. Ma, \emph{{Pretzelosity h(1T)**perpendicular and quark
  orbital angular momentum}},
  \href{http://dx.doi.org/10.1103/PhysRevD.79.054008}{\emph{Phys. Rev.} {\bf
  D79} (2009) 054008}, [\href{https://arxiv.org/abs/0902.3718}{{\tt
  0902.3718}}].

\bibitem{Lorce:2011kn}
C.~Lorce and B.~Pasquini, \emph{{Pretzelosity TMD and Quark Orbital Angular
  Momentum}},
...
{\scshape HERMES} collaboration, G.~Schnell, \emph{{The Sivers and other
  semi-inclusive single-spin asymmetries at HERMES}}, {\emph{PoS} {\bf DIS2010}
  (2010) 247}.
...
\newblock \href{http://dx.doi.org/10.1142/9789814277785_0029}{DOI}.
...
{\scshape Jefferson Lab Hall A} collaboration, J.~Huang et~al.,
  ...
  from Deep Inelastic Scattering on a Transversely Polarized He-3 Target at
  $1.4 < Q^2 < 2.7 GeV^2$}},
...
{\scshape HERMES} collaboration, A.~Airapetian et~al., \emph{{Observation of a
  single spin azimuthal asymmetry in semiinclusive pion electro production}},
...
S.~Jawalkar et~al., \emph{{Semi-Inclusive $\pi_0$ target and beam-target
  asymmetries from 6 GeV electron scattering with CLAS}},
  \href{https://arxiv.org/abs/1709.10054}{{\tt 1709.10054}}.

\bibitem{Airapetian:2002mf}
{\scshape HERMES} collaboration, A.~Airapetian et~al., \emph{{Measurement of
  single spin azimuthal asymmetries in semiinclusive electroproduction of pions
  and kaons on a longitudinally polarized deuterium target}},
...
{\scshape CLAS} collaboration, H.~Avakian et~al., \emph{{Measurement of
  beam-spin asymmetries for pi + electroproduction above the baryon resonance
  region}}, \href{http://dx.doi.org/10.1103/PhysRevD.69.112004}{\emph{Phys.
  Rev.} {\bf D69} (2004) 112004},
  [\href{https://arxiv.org/abs/hep-ex/0301005}{{\tt hep-ex/0301005}}].
...
M.~Aghasyan, H.~Avakian, P.~Rossi, E.~De~Sanctis, D.~Hasch et~al.,
  \emph{{Precise Measurements of Beam Spin Asymmetries in Semi-Inclusive
  $\pi^0$ production}},
...
A.~V. Efremov, K.~Goeke and P.~Schweitzer, \emph{{Azimuthal asymmetries at
  CLAS: extraction of e**a(x) and prediction of A(UL)}},
...
\bibitem{Yuan:2003gu}
F.~Yuan, \emph{{The Beam single spin asymmetry in semiinclusive deep inelastic
  scattering}},
...
M.~Anselmino, A.~Efremov, A.~Kotzinian and B.~Parsamyan, \emph{{Transverse
  ...
  in double-spin asymmetry A(LL) in semi inclusive DIS}},
...
{\scshape HERMES} collaboration, A.~Airapetian et~al., \emph{{Single spin
  azimuthal asymmetries in electroproduction of neutral pions in semiinclusive
...
E.~De~Sanctis, W.~D. Nowak and K.~A. Oganesian, \emph{{Single spin azimuthal
  asymmetries in the `Reduced twist - three approximation'}},
...
K.~A. Oganessian, N.~Bianchi, E.~De~Sanctis and W.~D. Nowak,
  \emph{{Investigation of single spin asymmetries in pi+ electroproduction}},
...
L.~Gamberg, Z.-B. Kang and A.~Prokudin, \emph{{Indication on the
  process-dependence of the Sivers effect}},
...
M.~Gluck, E.~Reya, M.~Stratmann and W.~Vogelsang, \emph{{Models for the
...
A.~Buckley, J.~Ferrando, S.~Lloyd, K.~Nordström, B.~Page, M.~Rüfenacht et~al.,
  \emph{{LHAPDF6: parton density access in the LHC precision era}},
  \href{http://dx.doi.org/10.1140/epjc/s10052-015-3318-8}{\emph{Eur. Phys. J.}
  {\bf C75} (2015) 132}, [\href{https://arxiv.org/abs/1412.7420}{{\tt
  1412.7420}}].

\bibitem{Berthou:2015oaw}
B.~Berthou et~al., \emph{{PARTONS: PARtonic Tomography Of Nucleon Software}},
  \href{http://dx.doi.org/10.1140/epjc/s10052-018-5948-0}{\emph{Eur. Phys. J.}
  {\bf C78} (2018) 478}, [\href{https://arxiv.org/abs/1512.06174}{{\tt
  1512.06174}}].

\bibitem{Hautmann:2014kza}
F.~Hautmann, H.~Jung, M.~Krämer, P.~J. Mulders, E.~R. Nocera, T.~C. Rogers
  et~al., \emph{{TMDlib and TMDplotter: library and plotting tools for
  transverse-momentum-dependent parton distributions}},
  \href{http://dx.doi.org/10.1140/epjc/s10052-014-3220-9}{\emph{Eur. Phys. J.}
  {\bf C74} (2014) 3220}, [\href{https://arxiv.org/abs/1408.3015}{{\tt
  1408.3015}}].
