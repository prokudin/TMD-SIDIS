...
F_{UU}^{\cos\phi_h} &\stackrel{\rm WW}{=}&\frac{2M}{Q}\,{\cal C}\biggl[\phantom{-}
	 \omega^{\{1\}}_{\rm A}\,x\,h\,H_{1}^{\perp } 
   	\red{\bm-}\omega^{\{1\}}_{\rm B}\,x\,f^\perp D_1\biggr]
        \with{ }
...
are related to twist-2 collinear functions. The other four 
$f_{1T}^{\perp a},  \;h_1^{\perp a},\; h_{1T}^{\perp a}, \; H_1^{\perp a} \,$  
have no collinear counterparts, and we consider their (1)-moments.
% which are of special interest as they are related to the so-called twist-3 
% collinear functions \AP{REFERENCE} and enter into the evolution of TMDs 
% \AP{REFERENCE}. 
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.3\textwidth]{\FigPath/Fig02a-JLab-ratio-pT-v5.pdf}  
%\includegraphics[width=0.27\textwidth]{Fig-HERMES.pdf}
%\includegraphics[width=0.27\textwidth]{Fig-COMPASS-multiplicities.pdf}
...
(b) Example for HERMES multiplicities (to be created).
(c) Example for COMPASS multiplicities (to be created).}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption{\label{jlab_ALL} 
	$p_2 A_{LL}$ asymmetry compared with Jefferson Lab experimental 
...
for Jefferson Lab 12 GeV are plotted in Fig.~\ref{aut_f1t_jlab}.

\PS{\bf (We should plot here HERMES and/or COMPASS data, two samples
just for illustrative purposes. We can add a comment that Jefferson Lab 12
data will significantly improve the experimental constraints.)}
...
\caption{\label{aut_f1t_jlab} $A_{UT}^{\sin(\phi_h-\phi_S)}$  as a function of $ x $ (left panel), and   as a function of $P_{hT}$ (right panel) at typical kinematics of Jefferson Lab 12 GeV, $\la x\ra = 0.25$, $\la z\ra = 0.54$, $\la Q^2\ra = 2.4$ GeV$^2$.
}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Asymmetries $A_{UT}^{\sin(\phi_h+\phi_S)}=F_{UT}^{\sin(\phi_h+\phi_S)}/F_{UU}$
for Jefferson Lab 12 GeV are plotted in Fig.~\ref{aut_h1_jlab}.
...
\caption{\label{aut_h1_jlab} $A_{UT}^{\sin(\phi_h+\phi_S)}$  as a function of $ x $ (left panel), and   as a function of $P_{hT}$ (right panel) at typical kinematics of Jefferson Lab 12 GeV, $\la x\ra = 0.25$, $\la z\ra = 0.54$, $\la Q^2\ra = 2.4$ GeV$^2$.
}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$b^{(2)}_{\rm AB}=\red{4}M_Nm_h$ and $c^{(2)}_{\rm AB} = 4M_Nm_h$,
see App.~\ref{App:convol-details}.

Asymmetries $A_{UU}^{\cos(2\phi_h)}=F_{UU}^{\cos(2\phi_h)}/F_{UU}$  for  
Jefferson Lab 12 GeV are plotted in Fig.~\ref{auu_cos2phi_jlab}.
...
\caption{\label{auu_cos2phi_jlab} $A_{UU}^{\cos(2\phi_h)}$  as a function of $ x $ (left panel), and   as a function of $P_{hT}$ (right panel) at typical kinematics of Jefferson Lab 12 GeV, $\la x\ra = 0.25$, $\la z\ra = 0.54$, $\la Q^2\ra = 2.4$ GeV$^2$.
}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\PS{\bf (We should plot here HERMES and/or COMPASS data, two samples
just for illustrative purposes. We can add a comment that Jefferson Lab 12
data will significantly improve the experimental constraints. I would also
skip the Cahn effect for simplicity, and add a comment of the following
kind:)}

In the (to be updated) Fig.~\ref{auu_cos2phi_jlab} we plot only the
...
\begin{subequations}\ba
	...
	&=& x\sum_q e_q^2\,h_{1T}^{\perp (2) q}(x)\,H_1^{\perp(1)q}(z)\;  
	c^{(3)}\,\biggl(\frac{z} {\lambda^{1/2}}\biggr)^{\!\red{3}}\,,
\ea\end{subequations}
where $\lambda=z^2 \avkperp_{h_{1T}^\perp} + \avpperp_{H_1^\perp}$ and
$b^{(3)}=2M_N^2m_h$ and $c^{(3)}_{\rm  } = 3\sqrt{\pi} \,M_N^2m_h$,
see App.~\ref{App:convol-details}.

Asymmetries $A_{UT}^{\sin(3 \phi_h - \phi_S)}=F_{UT}^{\sin(3 \phi_h - \phi_S)}/F_{UU}$ for 
Jefferson Lab 12 GeV are plotted in Fig.~\ref{aut_h1tp_jlab}.

\

\PS{\bf (We should plot here COMPASS data for illustrative purposes. 
I would propose to add a comment of the type:)}
...
\caption{\label{aut_h1tp_jlab} $A_{UT}^{\sin(3 \phi_h - \phi_S)}$  as a function of $ x $ (left panel), and   as a function of $P_{hT}$ (right panel) at typical kinematics of Jefferson Lab 12 GeV, $\la x\ra = 0.25$, $\la z\ra = 0.54$, $\la Q^2\ra = 2.4$ GeV$^2$.
}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\includegraphics[width=0.45\textwidth]{\FigPath/h1lperp.pdf}
	...
	$u$- and $d$-flavor. 
	\PS{\bf (Please, remove sea quarks in left panel).}}
\end{figure}
...
\ee
which the parametrizations for $f^{\perp q}_{1T}$ and $g^{\perp q}_{1T}$
used in this work of course satisfy \PS{\bf (do they??)}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
\centering
\includegraphics[width=0.45\textwidth]{\FigPath/ALT_x.pdf}  
\includegraphics[width=0.45\textwidth]{\FigPath/ALT_PT.pdf}
	\caption{\label{g1t_jlab} 
	$A_{LT}^{\cos(\phi_h -\phi_S)}$  as a function of $ x $ (left panel), 
	and as a function of $P_{hT}$ (right panel) at typical kinematics 
	of Jefferson Lab 12 GeV, 
	$\la x\ra = 0.25$, $\la z\ra = 0.54$, $\la Q^2\ra = 2.4$ GeV$^2$.
	\newline\PS{Can we compare here to COMPASS data from 
	\cite{Kotzinian:2007uv}?}}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The asymmetry  $A_{LT}^{\cos(\phi_h -\phi_S)}$ as a function of $x$ and $P_{hT}$ 
for Jefferson Lab 12 GeV is plotted in Fig.~\ref{g1t_jlab}.
...
\begin{figure}[b!]
\centering
\includegraphics[width=0.45\textwidth]{\FigPath/AULsin2phi_x.pdf} 
\includegraphics[width=0.45\textwidth]{\FigPath/AULsin2phi_PT.pdf}
	\caption{\label{aul_jlab} 
	$A_{UL}^{\sin(2\phi_h)}$  as a function of $ x $ (left panel), and   
	as a function of $P_{hT}$ (right panel) at typical kinematics of 
	Jefferson Lab 12 GeV, $\la x\ra = 0.25$, $\la z\ra = 0.54$, 
	$\la Q^2\ra = 2.4$ GeV$^2$.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Notice that the Boer-Mulders function and $h^{\perp q}_{1L}$ obey the
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.45\textwidth]{\FigPath/ALTcosPhi_x.pdf} 
\includegraphics[width=0.45\textwidth]{\FigPath/ALTcosPhi_PT.pdf}
...
Jefferson Lab 12 GeV are plotted in Fig.~\ref{allcosphi_jlab}.
...
\begin{figure}[b!]
\centering
\includegraphics[width=0.4\textwidth]{figs/ALLcosphi_x.pdf} 
\includegraphics[width=0.4\textwidth]{figs/ALLcosphi_PT.pdf}
\caption{\label{allcosphi_jlab} $A_{LL}^{\cos\phi_h}$  as a function of $ x $ (left panel), and   as a function of $P_{hT}$ (right panel) at typical kinematics of Jefferson Lab 12 GeV, $\la x\ra = 0.25$, $\la z\ra = 0.54$, $\la Q^2\ra = 2.4$ GeV$^2$.
}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.45\textwidth]{\FigPath/AULsinphi_x.pdf} 
\includegraphics[width=0.45\textwidth]{\FigPath/AULsinphi_PT.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.45\textwidth]{\FigPath/ALTcos2phi_x.pdf} 
\includegraphics[width=0.45\textwidth]{\FigPath/ALTcos2phi_PT.pdf}
...
Department of Energy under Contract No.~DE-AC05-06OR23177 (A.P.) and by the National Science Foundation
...
from \cite{Gluck:2000dy} for the unpolarized PDF $f_1^a(x)$ and 
...
$Q^2=2.5\,{\rm GeV}^2$ typical for many currently available SIDIS data.
...
In the recent analysis \cite{Anselmino:2013lza} 
of HERMES multiplicities only flavor-independence of the widths was 
...
In this work we will explore the parameters described in
Table~\ref{Table:Gauss-paramaters}. 
\PS{(What do we mean? Which parameters do we use?? All of them???)}
...
\hline
  & & & & \cr
\ fit of \cite{Anselmino:2005nn} & \red{XX}, \red{XX}, \red{XX} 
	& $\sim 0.25$ 
	& $\sim 0.2$ 
	& -- \cr
\ fit of \cite{Schweitzer:2010tt} & 2.5, 0.1, 0.4 
	...
	& $0.16\pm0.01$ 
	& -- \cr
\ fit of \cite{Anselmino:2013lza} & \red{XX}, \red{XX}, \red{XX} 
	& $0.57\pm0.08$ 
	& $0.12\pm0.01$ 
	& -- \cr
\ fit of \cite{Signori:2013mda} & \red{XX}, 0.1, 0.5 
	& $\sim0.3$
	& $\sim0.18$ 
	& -- \cr
\ lattice \cite{Hagler:2009mb}  & XX, -- , --
	& 0.14--0.15   
	& -- 
	& 0.11-0.15 \cr
  & & & & \cr 
...
because a different gauge link was chosen, see Sec.~\ref{Sec-3.5:lattice}. 
...
where ${N}^a$, $\alpha$, $\beta$, and $M_T$ are parameters fitted to data that can be found in Table~\ref{fitparI}.
  ...
  We use Eq.~(\ref{eq:moments}) to calculate the first moment of $ h_{1T}^{\perp a}(x,\kperp^2)$
of Eq.~(\ref{eq:h1Tperp}) and obtain:
\be
h_{1T}^{\perp (1) a}(x) = \frac{h^{\perp a}_{1T}(x) M_{TT}^2 \avkperp}{2 (M_{TT}^2 + \avkperp)^2} =  \frac{h^{\perp a}_{1T}(x) \avkperp_{h_{1T}^\perp}^2}{2 M_{TT}^2 \avkperp} \, .
\ee

We can rewrite parameterization of pretzelosity   functions as

\ba
h_{1T}^{\perp q}(x,\kperp) =  h_{1T}^{\perp (1) q}(x)   \frac{2 M^2}{\pi \avkperp_{h_{1T}^\perp}^2} e^{-\kperp^2/{\avkperp_{h_{1T}^\perp}}}
...
\newpage
\section{Convolution integrals and expressions in Gaussian Ansatz \red{(NEW)}}
\label{App:factor}
...
\begin{alignat}{6}
	...
	\;&\stackrel{\rm Gauss}{=}&\;
	\frac{n!\,\la \kperp^2\ra^n}{ 2^n\,M_N^{2n}}\,, \red{f(x)}\;\;\;\nonumber\\
	D^{(n)}(z) &=& \int d^2\bfpperp
	\left(\frac{\pperp^2}{2 z^2 m_h^2}\right)^{\!\!n}\; D(z, \pperp^2)
	\;&\stackrel{\rm Gauss}{=}&\;
	\frac{n!\,\la \pperp^2\ra^n}{ 2^n_{ }z^{2n}_{ }\,M_\red{h}^{2n}}\red{D(z)}\,. \;\;\;
	\label{eq:moments}
\end{alignat}
...
read \PS{\bf (working equation, to be removed)}
...
\item	I reshuffled some subsections with good intentions.
	Please, see whether you like it.
