...
% section 5.4 - is transversity really the only information on the tensor charge. how about lattice?
% FIgure 7: is it the <y> asymmetry (including prefactors)?
% section 5.6 (lines 819 ff) partially repetition (cubic Phperp suppression) and need reference to HERMES plot.
% figure 16 claims to be A_{LL}^{\cos\phi} but is just A_{LL}
...
\preprint{\red{version ``00z''}; \today; JLAB-THY-18-XXXX}
...
%Bakur's corrections: 
%	\bp{green-blue color, command: {$\backslash{\rm bp}\{\dots\}$}}\\
%Gunar's corrections: 
...
and depend on two independent variables: for TMDs, 
on the fraction $x$ of nucleon 
momentum carried by the parton and intrinsic transverse momentum $\kperp$ 
of the parton. For FFs, on the fraction $z$ of the parton momentum 
transferred to the hadron for and the transverse momentum of the hadron
...
\ee
and the asymmetries are defined as follows
\be
...
functions in this approximation and the Gaussian Ans\"atze which we use
...
\subsection{Gaussian Ans\"atze for TMDs and FFs}
\label{Sec-4.3:evaluation}

In this work we will use the so-called Gaussian Ans\"atze for the TMDs and FFs.
This Ans\"atze, which for a generic TMD or FF is given by
...
with this Ans\"atze. Far more important is the fact that it works 
...
Of course this Ans\"atze is only a rough approximation. For instance, 
...
scale in the process, then the Ans\"atze works quantitatively
very well. The most recent and detailed tests were reported in 
\cite{Schweitzer:2010tt}, where the Gaussian Ans\"atze was shown to
...
The Gaussian Ans\"atze is approximately compatible with 
...
Gaussian Ans\"atze is not compatible with the WW-type approximations:
...
the Gaussian Ans\"atze and WW-type approximations literally, one clearly
...
Ans\"atze and the WW-type approximations can be reconciled.
...
integrals in Gaussian Ans\"atze, and use then WW-type approximations.
...
\subsection{\boldmath Leading-twist $F_{UU}$ and Gaussian Ans\"atze}
\label{Sec-5.1:FUU-basis}

As explained in Sec.~\ref{Sec-4.3:evaluation} the Gaussian Ans\"atze is chosen
...
The Gaussian Ans\"atze for the unpolarized TMD and FF 
...
Figure~\ref{FUU-show-pT-dependence} illustrates how the Gaussian Ans\"atze
...
technical details on the Gaussian Ans\"atze (App.~\ref{App:factor}).
...
\subsection{\boldmath Leading-twist $A_{LL}$ and first test of Gaussian Ans\"atze
	in polarized scattering}
\label{Sec-5.2:FLL-basis}

The Gaussian Ans\"atze is useful in unpolarized case 
\cite{Anselmino:2005nn,Collins:2005ie,D'Alesio:2007jt,Schweitzer:2010tt,
...
with the Gaussian Ans\"atze within uncertainties. Second, the lattice 
...
The $P_{hT}$--dependences of the data are equally well described, which
...
data are compatible with the Gaussian Ans\"atze also in this case. 
...
\caption{\label{aut_h1_jlab}  Collins asymmetry 
	$A_{UT,  \langle y\rangle}^{\sin(\phi_h+\phi_S)}$ from proton target as function of $ x $ 
...
\caption{\label{auu_cos2phi_jlab} The asymmetry 
	$A_{UU, \langle y\rangle}^{\cos(2\phi_h)}$ from proton target as function of $ x $ 
...
Asymmetries  for $A_{UU, \langle y\rangle}^{\cos(2\phi_h)}=(1-y)F_{UU}^{\cos(2\phi_h)}/((1-y + y^2/2)F_{UU})$
HERMES \cite{Airapetian:2012yg} and $A_{UU}^{\cos(2\phi_h)}=F_{UU}^{\cos(2\phi_h)}/F_{UU}$ COMPASS \cite{Adolph:2014pwc}
...
\includegraphics[width=0.45\textwidth]{\FigPath/AUTPretzelosity_HERMES_x.pdf}

\caption{\label{aut_h1tp_jlab} $A_{UT}^{\sin(3 \phi_h - \phi_S)}$  
	as a function of $x$ from preliminary COMPASS \cite{Parsamyan:2010se} 
	and $A_{UT,\langle y\rangle}^{\sin(3 \phi_h - \phi_S)}$ HERMES \cite{Schnell:2010zza} in comparison to the best fit 
...
introduced by choosing a certain fit Ans\"atze which however can be 
...
$h_{1L}^{\perp q}$ the opposite configuration, namely transversely 
...
We assume for $g^{\perp}_{1T}$ the Gaussian Ans\"atze as shown 
in Eq.~(\ref{eq:g1t}) of App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs}, 
...
In the Gaussian Ans\"atze the structure function $F_{LT}^{\cos(\phi_h -\phi_S)}$ 
has the form
\begin{subequations}\ba
...
This asymmetry was measured at Jefferson Lab~\cite{Huang:2011bc}, COMPASS~\cite{Parsamyan:2015dfa} and HERMES~\cite{Pappalardo:2011cu,Pappalardo:2012zz} (for the latter two experiments only preliminary results are available so far).
 Fig.~\ref{cosphihphis} 
shows the preliminary results from the 2010 COMPASS data \cite{Parsamyan:2015dfa}. 
We approximate the 
...
Our results are also compatible with the Jefferson Lab data, which was 
taken with a neutron ($^3$He) target \cite{Huang:2011bc} and has 
larger statistical uncertainty than the preliminary COMPASS data. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[b!]
\centering
...
\includegraphics[width=0.45\textwidth]{\FigPath/LT_Pr10_cosphihphis_pim}
	...
	Leading-twist $A_{LT}^{\cos(\phi_h-\phi_S)}$ 
	 in comparison to the preliminary COMPASS data \cite{Parsamyan:2015dfa}.
	}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs}, with
...
\begin{figure}[b!]
\centering
\includegraphics[width=0.8\textwidth]{\FigPath/A_UL2_Pr0711_Zgt02.pdf} \quad
\includegraphics[width=0.45\textwidth]{\FigPath/AULsin2phi_JLAB_x.pdf} 
\vspace{-5mm}
...
\cite{Parsamyan:2015dfa}, and Jefferson Lab \cite{Avakian:2010ae,Jawalkar:2017ube}.
...
and $h^{\perp q}_{1L}$ and Gaussian Ans\"atze for all TMDs), and (ii) we deal 
...
We assume the Gaussian Ans\"atze for $g_T^a(x,\kperp)$ shown in
Eq.~(\ref{eq:gtnew}) of App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs} 
...
Sec.~\ref{Sec-4.4:evaluation}: first assume Gaussian Ans\"atze, then apply 
...
convolution integral with Gaussian Ans\"atze. The result is an
...
Fig.~\ref{altcosphis} shows our predictions
...
$g_T^{\perp a}(x,\kperp)$, for which we assume a Gaussian Ans\"atze according to 
Eq.~(\ref{eq:gtperpnew}) in App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs},
...
\caption{\label{altcos2phihphis} $A_{LT}^{\cos(2\phi_h - \phi_S)}$ 
	as a function of $ x $ from a proton target in comparison to the preliminary COMPASS data \cite{Parsamyan:2015dfa}. The smallness
...
in App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs} with 
...
$h_L^q(x,\kperp)$, for which we assume the Gaussian Ans\"atze 
(\ref{eq:hLnew}) in App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs}
...
\includegraphics[width=0.9\textwidth]{\FigPath/A_UL1_Pr0711_Zgt02.pdf}
\caption{\label{aulsinphi_jlab} 
	...
	Lower panel: preliminary $h^\pm$ COMPASS data \cite{Parsamyan:2015dfa}.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs} and relating 
...
\includegraphics[width=0.45\textwidth]{\FigPath/AUTsinphiS_HERMES_x.pdf}
\caption{\label{autsinphis} 
	...
	comparison to preliminary COMPASS~\cite{Parsamyan:2015dfa} and HERMES data \cite{Schnell:2010zza}.}
\end{figure}
...
Preliminary 
HERMES \cite{Schnell:2010zza} (Fig.~\ref{autsinphis}) and 
COMPASS \cite{Parsamyan:2015dfa} (Fig.~\ref{autsinphis}) data 
...
\includegraphics[width=0.45\textwidth]{\FigPath/UT_Pr10_sin2phihphis_pip}
\includegraphics[width=0.45\textwidth]{\FigPath/AUTsin2PhiminusPhiS_HERMES_x.pdf}
\caption{\label{autsin2phihphis} 
	Subleading--twist $A_{UT}^{\sin(2 \phi_h - \phi_S)}(x)$  in comparison with preliminary COMPASS data~\cite{Parsamyan:2015dfa}, and 	
	subleading--twist $A_{UT, \langle y \rangle}^{\sin(2\phi_h-\phi_S)}(x)$ in comparison to the 
...
to preliminary COMPASS \cite{Parsamyan:2015dfa} and 
...
Using the Gaussian Ans\"atze for $f^\perp(x,\kperp)$ in Eq.~(\ref{eq:fperpnew})
of App.~\ref{App-B:Gauss-Ans\"atze-non-basis-TMDs} we obtain 
\begin{subequations}\begin{alignat}{1}
...
\caption{\label{auucosphi_jlab} 
	...
	panel: the same asymmetry $A_{UU, \langle y \rangle}^{\cos\phi_h}$ for $\pi^\pm$ from HERMES \cite{Airapetian:2012yg}.}
\end{figure}
...
Ans\"atze for TMDs) in our approach.
...
Incidently, these asymmetries include the very first non-zero azimuthal 
...
($F_{UL}^{\sin\phi_h}$), and the very first non-zero beam single-spin 
asymmetry measured in SIDIS ($F_{LU}^{\sin\phi_h}$).
...
and $D_1(z,P_\perp^2)$ we use the Gaussian Ans\"atze in 
...
Gaussian Ans\"atze will remain a useful {\it approximation} as long as one is 
...
\section{Convolution integrals and expressions in Gaussian Ans\"atze}
\label{App:factor}
...
\subsection{Gaussian Ans\"atze}

For a generic TMD and FF the Gaussian Ans\"atze is given by 
\be
    f^a(x,\kperp^2)=
...
\label{App-B:Gauss-Ans\"atze-non-basis-TMDs}

Having discussed the Gaussian Ans\"atze for the 8 basis functions
