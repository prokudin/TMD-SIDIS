...
and depend on two independent variables: \bp{for TMDs,} 
on the fraction $x$ of nucleon 
momentum carried by parton and intrinsic transverse momentum $\kperp$ 
of the parton, \bp{For FFs, on the fraction $z$ of the parton momentum 
...
the twist-3 $g_T^a(x)$ and $h_L^a(x)$ can be \bp{respectively} expressed 
in terms of contributions from twist-2 $g_1^a(x)$ and $h_1^a(x)$, and 
additional quark-gluon-quark ($\bar{q}gq$) correlations or current 
quark mass terms \cite{Wandzura:1977qf,Jaffe:1991ra}
...
\bp{performed or in preparation at HERMES, COMPASS,} Jefferson Lab (JLab) with 
...
\bp{Our predictions, whether confirmed or not supported by current and 
...
technical details. 
	In App.~C we describe \bp{an open source package implemented 
	in \texttt{mathematica}~\cite{Mathematica} (already available) 
	and \texttt{Python} (to be released in the near future) which is 
	made publicly available on \texttt{github.com}.}\footnote{\href{%
...
\bp{defines} the z-axis, and $l^\prime$ points in the direction of the x-axis 
...
\bp{where $M_Z$ denotes the mass of the $Z^0$ electroweak gauge boson.}
...
\bp{characterizing the overall orientation of the lepton scattering plane 
...
targets is chosen to be the direction of $S_T$. In DIS limit 
...
longitudinal polarization of the virtual photon (and \bp{another} the 
...
\ee
and the asymmetries are defined as \bp{follows}
\be
	A_{XY}^{\rm weight}\equiv A_{XY}^{\rm weight}(x,z,\Phperp)=
	\frac{F_{XY}^{\rm weight}(x,z,\Phperp)}{F_{UU}(x,z,\Phperp)}.
\ee
Here the first \bp{subscript} $X=U(L)$ denotes the unpolarized beam
(longitudinally polarized beam with helicity $\lambda$). The second 
\bp{subscript} $Y=U(L,T)$ refers to the target which can be unpolarized
...
\bp{where $M_N$ is the nucleon mass.} In our treatment to $1/Q^2$ 
accuracy we neglect two structure functions due to longitudinal virtual 
photon polarization which contribute at order ${\cal O}(M_N^2/Q^2)$ in the 
...
\includegraphics[width=0.3\textwidth]{\FigPath/Screenshot-Fig-COMPASS-A_UT_Pr10-left-panel-up.png} \ 
\includegraphics[width=0.3\textwidth]{\FigPath/Screenshot-Fig-COMPASS-A_UT_Pr10-left-panel-down.png}
\includegraphics[width=0.3\textwidth]{\FigPath/AUTPretzelosity_HERMES_x.pdf}

\caption{\label{aut_h1tp_jlab} $A_{UT}^{\sin(3 \phi_h - \phi_S)}$  
	as a function of $x$ from preliminary COMPASS \cite{Parsamyan:2010se} 
	and HERMES \cite{Schnell:2010zza} in comparison to the best fit 
	...
	\TD{\sf screenshot of {\tt Fig-COMPASS-A\_UT\_Pr10.pdf} 
	which was previously shown in Fig.~\ref{autsinphi_jlab}. 
	Bakur: please, can you produce such plot with Alexei's 
	``best fit'' from \cite{Lefky:2014eia}?}}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$h_{1L}^{\perp q}$ the opposite \bp{configuration}, namely transversely 
...
\includegraphics[width=0.9\textwidth]{\FigPath/LT_Pr10.pdf}
...
\begin{figure}[b!]
\centering
\includegraphics[height=3.1cm]{\FigPath/A_UL2_Pr0711_Zgt02.pdf} \quad
\includegraphics[height=3.0cm]{\FigPath/AULsin2phi_JLAB_x.pdf} 

\vspace{-5mm}
 
	\caption{\label{aul_jlab} 
	$A_{UL}^{\sin(2\phi_h)}$ vs $x$ from the WW-type 
	approximation in comparison to data from
	HERMES \cite{Airapetian:1999tv}, 
	COMPASS \cite{Parsamyan:2015dfa} (left panel), and
...
\vspace{1cm}
\includegraphics[width=0.6\textwidth]{\FigPath/A_LL0_Pr0711_Zgt02.pdf}
...
\includegraphics[width=0.7\textwidth]{\FigPath/A_UL1_Pr0711_Zgt02.pdf}
...
\ps{Overall, the WW-type approximation is not adequate to catch the
...
dependence which is distinct from that of the Collins effect.}
...
for $F_{UT}^{\sin\phi_S}$ is given in terms a single term
...
The third interesting feature is the occurrence of a term which drops 
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.4\textwidth]{\FigPath/AUTsinphiS_HERMES_x.pdf}
\caption{\label{Fig-AUTsinphiS-HERMES} 
 	Subleading--twist asymmetry $A_{UT}^{\sin\phi_S}(x)$, which 
	is predicted to vanish in the WW-type approximation, in 
	comparison to preliminary HERMES data \cite{Schnell:2010zza}.}
\end{figure}
...
\ps{Preliminary 
HERMES \cite{Schnell:2010zza} (Fig.~\ref{Fig-AUTsinphiS-HERMES}) and 
COMPASS \cite{Parsamyan:2015dfa} (Fig.~\ref{autsinphi_jlab}) data 
...
\begin{figure}[b]
\centering
\includegraphics[width=0.6\textwidth]{\FigPath/UT_Pr10.pdf}
\includegraphics[width=0.3\textwidth]{\FigPath/AUTsin2PhiminusPhiS_HERMES_x.pdf}
...
\includegraphics[width=0.37\textwidth]{\FigPath/AUUcosphi_HERMES_x.pdf}
...
COMPASS data. 
\ps{The WW-type approximation tends to overstimate the data at
...
helicities \ps{$\pm1$ or $0$ (with the latter denoting an unpolarized beam).}
