...
\preprint{\red{version ``01a''}; \today; JLAB-THY-18-XXXX}
...
Notice that Ref.~\cite{Bacchetta:2006tn} uses four-vector notations for transverse vectors, while in this paper we always utilize two-vectors for transverse vectors, such that e.g. ${\kperp^2}_{\rm our} = -{{\bm p}_T^2}_{\tiny \rm Ref.\cite{Bacchetta:2006tn}}$
...
in terms of the basis functions. \AP{Notice that SIDIS alone is of course not sufficient to determine the basis
...
%functions uniquely: \AP{8 twist-2 TMDs (or 6 ``basis'' TMDs) and 2 twist-2 TMD FFs appear in 15 structure functions (8 twist-2 and 5 sub-leading). %Definitely hadron production in $e^+e^-$ annihilation data is needed, each FF couples with 4 different twist-2 TMD PDFs, and so for each FF we have a case with 5 unknowns accessible in 4 twist-2 observables.
%It is crucial to take advantage of  Drell--Yan process for TMDs
%and other processes 
%play also important roles.}
...
Aghasyan:2011ha,Adolph:2014pwc,Gohn:2014zbz}, \AP{these 2\% come after $1/Q$ suppression. Unsuppressed effect at large $x$ at COMPASS would reach 10\%.} 
...
($F_{UL}^{\sin\phi_h}$), and \AP{is the only single-spin beam-spin asymmetry} measured in SIDIS ($F_{LU}^{\sin\phi_h}$).
