...
the Wandzura--Wilczek-type approximation \ps{which consists in a systematic
neglect of $q\bar q g$-terms with respect to $\bar{q}q$-correlators.}
...
in the near-term (JLab 12, \ps{COMPASS}) or proposed in the long-term 
...
\ps{or not be supported by data. The first case will call for dedicated 
theoretical studies to explain why the pertinent $\bar{q}gq$-terms are 
so small. The second case will call for explanations why sometimes 
$\bar{q}gq$-terms are so sizable. In this way our work will help to guide 
further developments. In either case our results will deepen the
understanding of the quark-gluon correlations inside hadrons.}
...
\vspace{-7mm}
\centering
	\includegraphics[width=7.2cm]{\FigPath/Fig02a-kin-SIDIS-UT.eps}
...
\ps{In principle the cross section is also differential with respect to the} 
azimuthal angle $\psi_l$ characterizing the overall orientation of the 
lepton scattering plane in \ps{a fixed} lab frame.
%
...
lepton--quark scattering \ps{subprocess} cross section
...
At subleading order \ps{in the $1/Q$ expansion} one has
\ba\hspace{-1cm}
...
\ea\end{subequations}
Finally, \ps{integrating over $z$, summing over hadrons, and} 
...
known about them.
The basic idea of the approximations is simple. One \ps{uses QCD
...
with respect to \ps{the leading} $\bar{q}q$-terms with a useful accuracy,
...
\ps{A prefactor of $x$ is provided in (\ref{Eq:WW-original-e})}
...
The data are from \cite{Anthony:2002hy,Abe:1998wq}. The \ps{estimate of the 
theoretical uncertainty} is described in the text.
\red{Gunar: good idea to include HERMES data \cite{Airapetian:2011wu}!
Please send us the table with the data points.}}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$40\,\%$-``uncertainty band'' estimated in \cite{Accardi:2009au}. This however 
...
WW-type approximations from \ps{models}.
...
in this model are quarks, antiqarks and Goldstone bosons which are 
...
\ps{In some cases, however, we know in advance that the WW-type approximations 
have limitations, which we discuss in the next section.}
...
\ps{The WW-type approximations have limitations} in the description of 
...
WW-type approximation (\ref{Eq:WW-type-fT}) literally \ps{means}
$x\int d^2 k_\perp\,f_T^a(x,k_\perp)\,\ps{\stackrel{!?}{=}}
-f_{1T}^{\perp(1)a}(x)\neq0$ 
...
\section{SIDIS in WW-type approximation \ps{and Gaussian model}}
\label{Sec-4:SIDIS-in-WW-approximation}

In this section we consequently apply the WW- and WW-type approximation
to SIDIS, \ps{and describe our procedure to evaluate the structure 
...
(\ref{Eq:WW-type-5}--\ref{Eq:WW-type-8})
the situation is different. In these approximations a twist-3 TMD
is related to a transverse moment of twist-2 TMD. In such cases the 
Gaussian Ansatz is not compatible with the WW-type approximations:
for instance, the approximation (\ref{Eq:WW-type-5}) relates
$xg_T^q(x,\kperp^2)\approx\frac{k_\perp^2}{2M_N^2}\,g_{1T}^{q}(x,\kperp^2)$,
i.e.\ if $g_{1T}^q(x,\kperp^2)$ was exactly Gaussian then 
$g_T^q(x,\kperp^2)$ certainly could not be Gaussian. If one would take
...
(\ref{Eq:WW-type-5}--\ref{Eq:WW-type-8}) in the limit $k_\perp\to 0$ is 
...
Eqs.~(\ref{Eq:WW-type-5}--\ref{Eq:WW-type-8}).
...
Notice that in principle one could choose a different order
of the steps: first use WW-type approximations and then
solve the convolution integrals assuming  Gaussian Ansatz.
This would constitutes ``alternative scheme'' with 
in general different (and bulkier) analytical expressions.
Given the modest understanding of factorization for twist-3 
TMDs it is not possible to discriminate which scheme might be a
better approximation. But we convinced ourselves that the two 
schemes yield similar results for the structure functions, and the 
differences are numerically within the accuracy expected for
this approach.

When following the above-described scheme (solving first the 
convolutions integrals and using then WW-type approximations) 
we will see that the results for the structure functions can 
be conveniently expressed in terms of the basis TMDs or their
adequate transverse moments. We will come back to this point. 

Finally, let us recall that the WW-type approximations 
(\ref{Eq:WW-type-fT},~\ref{Eq:WW-type-last}) for those
twist-3 T-odd TMDs which are subject to the sum rules
in Eq.~(\ref{Eq:sum-rules-T-odd}) require special care,
see the discussion in Sec.~\ref{Sec-3.8:limitations}. 
...
This has two important advantages. First, this step simplifies the
the Gaussian Ansatz expressions, and the Gaussian width parameters are
...
\begin{figure}[b!]
\centering
\includegraphics[height=3cm]{\FigPath/Fig02a-JLab-ratio-pT-v5-NEW.pdf}  \quad
\includegraphics[height=3cm]{\FigPath/hermes_pip.pdf} \quad
\includegraphics[height=3cm]{\FigPath/compass_hp.pdf} 
\caption{\label{FUU-show-pT-dependence}
Examples of the 
	description of transverse momenta of hadrons in unpolarized SIDIS. 
Left panel: 
	cross section for $\pi^+$ production normalized with 
...
In Fig.~\ref{FUU-show-pT-dependence} (a) we show CLAS data 
\cite{Osipenko:2008aa} on $\pi^+$ production from proton 
with a 5.75 GeV beam which shows a Gaussian $P_{hT}$-dependence
over 2 orders of magnitude, see \cite{Schweitzer:2010tt} for details.
In Fig.~\ref{FUU-show-pT-dependence} (b) we plot 
the HERMES multiplicity~\cite{Airapetian:2012ki}
...
for $\pi^+$ production on the proton target.
In Fig.~\ref{FUU-show-pT-dependence} (c) we plot the COMPASS multiplicity 
...
The $F_{UT}^{\sin(\phi_h-\phi_S)}$ \ps{structure function} is related to the 
Sivers function~\cite{Sivers:1989cc}, which describes the \ps{distribution}
...
\begin{figure}[b!]
\centering
\includegraphics[width=0.43\textwidth]{\FigPath/AUTSivers_x.pdf}  \hspace{5mm}
\includegraphics[width=0.43\textwidth]{\FigPath/AUTSivers_PT.pdf}
...
\caption{\label{aut_h1tp_jlab}  Comparison of calculation of $A_{UT}^{\sin(3 \phi_h - \phi_S)}$  using fit results of Ref.~\cite{Lefky:2014eia} as a function of $ x $ (left panel), and   as a function of $P_{hT}$ (right panel) at $\la x\ra = 0.05$, $\la z\ra = 0.36$, $\la Q^2\ra = 3.6$ GeV$^2$ and preliminary COMPASS data \cite{Parsamyan:2010se}.
}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.3\textwidth]{\FigPath/g1t.pdf} \quad
\includegraphics[width=0.3\textwidth]{\FigPath/h1Lperp.pdf}
...
\includegraphics[width=0.9\textwidth]{Fig-COMPASS-A_LT_Pr10.pdf}
	...
	on which we comment in Sec.~\ref{Sec-7.5:FLTcos2phi-phiS}.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The left panel of Fig.~\ref{g1t_jlab} shows the 2010 COMPASS data 
for $A_{LT}^{\cos(\phi_h -\phi_S)}$ \cite{Parsamyan:2015dfa} measured 
with 160 GeV longitudinally polarized muons impinging on a 
transversely polarized proton (NH$_3$) target \cite{Goertz:2002vv}. 
As at COMPASS about $90\,\%$ of charged hadrons are $\pi^\pm$, in our 
calculation we approximate charged hadron fragmentation functions by 
$D_1^a(z)$ of pions, see App.~\ref{App:basis-f1-D1}.
Clearly, our results describe the data rather well within the 
experimemental uncertainties. We observe that the WW-type 
approximation works in this case. For comparison also results from 
\cite{Kotzinian:2006dw,Kotzinian:2008fe,Boffi:2009sh} are shown.
The results shown in middle and right panel of Fig.~\ref{g1t_jlab} will be 
discussed in Secs.~\ref{Sec-7.2:FLTcosphiS},~\ref{Sec-7.5:FLTcos2phi-phiS}.
...
We assume Gaussian form for the Kotzinian-Mulders function 
$h_{1L}^{\perp a}$, see Eq.~(\ref{eq:h1l_final}) in
App.~\ref{App-B:Gauss-Ansatz-non-basis-TMDs}. From 
Eq.~(\ref{Eq:WW-approx-h1L}) we obtain the WW-type estimate
for $h_{1L}^{\perp(1) a}(x)$ shown in Fig.~\ref{g1t_h1l_functions}. 
For the Gaussian width we assume $\avkperp_{h_{1L}^\perp} = \avkperp_{h_{1}}$ 
...
\begin{figure}[b!]
\centering
\includegraphics[height=3.4cm]{Fig-COMPASS-A_2_Pr0711_Zgt02.pdf} \quad
\includegraphics[height=3.4cm]{\FigPath/AULsin2phi_JLAB_x.pdf}  
	...
	experiments (left panel) and at Jefferson Lab 
...
studied at HERMES \cite{Airapetian:1999tv,Airapetian:2002mf}, 
COMPASS \cite{Parsamyan:2015dfa}, and JLab \cite{Jawalkar:2017ube}.
In the left panel of Fig.~\ref{aul_jlab} the results for proton
target are shown. At HERMES 27.6 GeV positrons from the HERA polarized 
positron storage ring at DESY scattering off a nuclear-polarized hydrogen 
internal gas targets \cite{Stock:1994vv} in the kinematics
$1\,{\rm GeV}^2 < Q^2 < 15 \,{\rm GeV}^2$, $W > 2\,{\rm Gev}$,
$0.023 < x < 0.4$ and $y < 0.85$. The data for $\pi^\pm$ from 
proton are shown in Fig.~\ref{aul_jlab}. The COMPASS data show 
the asymmetry for charged hadrons, which in practice are mainly
pions. Since $y$-dependent prefactors were included in the
analyses (see Sec.~\ref{Sec-2.1:SIDIS+structure-functions}),
the HERMES data is adequately rescaled (``$D(y)$--rescaled'').

\ \\
\red{\large\bf Stopping here. Sending to Alexei!
Continue from here.}
...
We discussed WW-type approximations for
$g^{\perp q}_{1T}$ and $h^{\perp q}_{1L}$ and the two
twist-2 structure functions amenable to this approximation
in Secs.~\ref{Sec-6.1:FLTcosphi-phiS} and~\ref{Sec-6.2:FULsin2phi}.
Before focusing on twist-3 let us pause 
and revisit positivity bounds \cite{Bacchetta:1999kz} 
which offer unique cross checks. 
...
\begin{subequations}\ba
	...
	(h_{1}^{\perp q}(x,\kperp^2))^2\right] 
	& \le & (f_{1}^{q}(x,\kperp^2))^2 \, ,\\
	\frac{\kperp^2}{M^2}\left[ (g^{\perp q}_{1T}(x,\kperp^2))^2 
	+ (f_{1T}^{\perp q}(x,\kperp^2))^2\right] 
	& \le & (f_{1}^{q}(x,\kperp^2))^2 \; . 
\ea\end{subequations}
From these inequalities and our Gaussian Ans\"atze in 
Eqs.~(\ref{Eq:Gauss-f1}, \ref{Eq:Gauss-h1}, \ref{Eq:Gauss-f1Tperp}, 
\ref{Eq:Gauss-h1perp}, \ref{eq:g1t}, \ref{eq:h1l_final}) one obtains 
(after $k_\perp$-integration and mutliplication by $16\pi M_N^2$) 
the inequalities
\begin{subequations}\ba
	(f_{1}^{q}(x))^2
	- \frac{4M^2 (f_{1T}^{\perp q}(x))^2}{\avkperp_{f_{1T}^\perp} } 
	- \frac{4M^2 (g_{1T}^{\perp q}(x))^2}{\avkperp_{g_{1T}^\perp} } &\ge& 0
	\;, \label{eq:positivity}\\
	(f_{1}^{q}(x))^2 
	- \frac{4M^2 (h_{1}^{\perp q}(x))^2}{\avkperp_{h_{1}^\perp} } 
	- \frac{4M^2 (h_{1L}^{\perp q}(x))^2}{\avkperp_{h_{1TL}^\perp} } &\ge& 0\,.
	\mbox{\red{\ (introduce (1)-moments!)}}
...
\begin{figure}[b!]
\centering
\includegraphics[width=0.3\textwidth]{\FigPath/g1t_positivity.pdf}  \quad
\includegraphics[width=0.3\textwidth]{\FigPath/h1lperp_positivity.pdf}  
...
The left-hand-sides of these inequalities are plotted in 
Fig.~\ref{h1l_pos} which shows that positivity is satisfied 
in 3 out of 4 cases: only (\ref{eq:positivity1}) for $d$-flavor 
seems violated for $x>0.2$. 

At this point some remarks are in order.
First, the inequalities (\ref{eq:positivity},~\ref{eq:positivity1}) 
must be strictly satisfied by the {\it exact} leading--order QCD 
expressions for the TMDs. 
(For PDFs it is known that positivity can be preserved in some schemes 
and violated in other schemes. For TMDs not much is known about 
positivity conditions beyond leading order.)
However, here we do not deal with exact expressions for the TMDs
but we invoked two strong model assumptions: WW-type
approximations for $g^{\perp q}_{1T}$ and $h^{\perp q}_{1L}$, and 
Gauss Ansatz for all TMDs. This is a lot! Neverthelss we find 
that (\ref{eq:positivity},~\ref{eq:positivity1}) are respected 
for $x<0.2$. This is a non-trivial cross check of our aproach. 
Second, available data constraining the TMDs are mostly at $x<0.2$. 
There are in fact only few $x$-bins above $x>0.2$. For $x>0.4$ the 
available parameterizations of TMDs are practically unconstrained.
Third, the determination of the Boer-Mulders function from 
Ref.~\cite{Barone:2015ksa} is far from ideal especially for $d$-flavor, 
and non-factorizing power-corrections (due to Cahn effect) may be at play. 

Considering that we deal with approximations (WW-type, Gauss) 
and considering current state of TMD-extractions, the 
positivity test in Fig.~\ref{h1l_pos} can be considered satisfactory. 
...
\begin{figure}[h!]
\includegraphics[width=0.8\textwidth]{Fig-COMPASS-A_LL_Pr0711_Zgt02.pdf}
\end{figure}

\begin{figure}[h!]
\includegraphics[width=0.8\textwidth]{Fig-COMPASS-A_UL1_Pr0711_Zgt02.pdf}
\end{figure}

\begin{figure}[h!]
\includegraphics[width=0.8\textwidth]{Fig-COMPASS-A_UT_Pr10.pdf}
\end{figure}
...
Notice that we could have used WW-type relation of Eq.~(\ref{Eq:WW-type-6}) 
to relate $g^{q}_{T}(x,\kperp^2)$ and $g^{\perp q}_{1T}(x,\kperp^2)$ however we 
choose not to do it. In fact Eq.~(\ref{Eq:WW-type-6}) suggests kinematical 
zero $g^{q}_{T}(x, 0) = 0$  which is not supported by the model calculations 
\cite{Avakian:2010br} and thus  Eq.~(\ref{Eq:WW-type-6}) can be used only 
in its integrated form. 
The Eq.~(\ref{Eq:WW-type-6}) is not exact and is not on the same footing as 
WW-approximation of Eq.~(\ref{Eq:WW-original1}), and we parametrize directly 
$g^{q}_{T}(x,\kperp^2)$ using the Gaussian shape. For illustrative purpose we 
show ``violation'' of  Eq.~(\ref{Eq:WW-type-6}) in Fig.~\ref{altcosphi_wwtype}.

The $F_{LT}^{\cos\phi_S}$ structure function then reads 
\begin{subequations}\ba
...
the Gaussian width $\avkperp_{g_T}=\avkperp_{g_1}$.\footnote{
	...
	assume then the Gaussian Ansatz for $g_{1T}^{\perp}(x,\kperp)$ 
	...
	employing the Gaussian Ansatz do not commute in general.
...
We compare our calculations to HERMES data \cite{Airapetian:2005jc} 
in Fig.~\ref{aulsinphi_jlab} (a) and to CLAS data in 
Fig.~\ref{aulsinphi_jlab} (c).
The asymmetries $A_{UL}^{\sin\phi_h}=F_{UL}^{\sin\phi_h}/F_{UU}$  as a function 
of $x$  for COMPASS is plotted in Fig.~\ref{aulsinphi_jlab} (b). 
...
\includegraphics[width=0.45\textwidth]{\FigPath/AULsinphi_JLAB_x.pdf} (c)
...
applying the WW-type approximation. Assuming Gaussian Ans\"atze for the TMDs
...
TMD $f_T^q(x,\kperp)$, and the Gaussian model only {\it artificially} 
...
an additional Gaussian parameter and use a double Gaussian to describe 
...
%------ BEGIN FIGURE 4: Phperp(z) AND Phperp^2(z) at HERMES ----------
        ...
        for this observable in the Gaussian model with parameters 
...
In this last asymmetry we encounter the first case with more than one 
surviving contribution in our treatment. Assuming Gaussian Ans\"atze for 
$f_T^{\perp q}(x,\kperp)$, $h_T^{\perp q}(x,\kperp)$ Eq.~(\ref{eq:hTperpnew}), 
$h_T^q(x,\kperp)$ Eq.~(\ref{eq:hTnew}),
\be
...
\caption{\label{autsin2phi_jlab} $A_{UT}^{\sin(2\phi_h- \phi_S)}$  for COMPASS
	\red{(GUNAR: here it says COMPASS, in text JLab)}}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This point has to be cautiously kept in mind. Future determinations 
of not only of the Boer-Mulders functions but also all other TMDs
from precision JLab data (and on long run from the EIC) will 
improve the situation, and allow to test the positivity constraints
more rigorously. In this way, the TMD leading-twist factorization
approach will also be validated.
...
\subsection{Gaussian Ans\"atze used in this work}
...
following 8 TMDs as listed below:
\begin{subequations}\begin{align}
	...
	h_{1L}^{\perp a}(x,\kperp^2) 
	  & = 	h_{1L}^{\perp (1) a}(x)\,
		\frac{2 M_N0^2}{\pi{\avkperp_{h_{1L}^\perp}^2}}\,
